...
impl EntityInputHandler for SearchInput {
    ...
    fn replace_and_mark_text_in_range(
        ...
        self.selected_range = selected_range_utf16
            .as_ref()
...
impl Element for SearchTextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), hsla(0., 0., 0.42, 1.))
        } else {
            (content, style.color)
        };
        let run = TextRun {
    ...
    fn paint(
        ...
        let line = prepaint.line.take().expect("search input line is shaped");
        line.paint(bounds.origin, window.line_height(), window, cx)
            .expect("search input text paints");
